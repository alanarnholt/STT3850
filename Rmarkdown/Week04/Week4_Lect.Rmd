...
```{r, echo = FALSE}
knitr::include_graphics("./pics/skim.png", dpi=600)
```
...
- $-1\leq r \leq 1$
    ...
    - +1 indicates a perfect positive linear relationship: As the value of one variable goes up, the value of the other variable tends to go up as well in a linear fashion.
- $r_{x,y}= r_{y,x}$
...
```{r out.height = '45%',out.width = '60%'}
ggplot(evals_ch5, aes(x = bty_avg, y = score)) +
  geom_point(size = 3, color = "purple", alpha = 0.2) +
  labs(x = "Beauty Score", y = "Teaching Score",
...
# Or
broom::glance(score_model)$r.squared
```
\normalsize
