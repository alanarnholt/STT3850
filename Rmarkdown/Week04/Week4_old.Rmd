...
```{r warning=FALSE, message=FALSE,out.height = '85%',out.width = '80%', fig.align='center'}
...
- A hypothesis test of: $H_0: \beta_0=\beta_0^*\quad vs \quad H_a: \beta_0\neq\beta_0^*$, 

is obtained  by computing
$$ t=\frac{b_0-\beta_0^*}{S(b_0)}\sim t_{n-2}, \quad \text{under } H_0$$

Then, reject $H_0$ if $|t|>t_{n-2, \alpha/2}$.




Similarly, a hypothesis test of $H_0: \beta_1=\beta_1^*\quad vs \quad H_a: \beta_1\neq\beta_1^*$, 

is obtained  by computing
$$ t=\frac{b_1-\beta_1^*}{S(b_1)}\sim t_{n-2}, \quad \text{under } H_0$$
...
Now we construct a $95\%$ confidence interval for $\beta_1$.

\tiny
```{r echo=TRUE,warning=FALSE, message=FALSE,out.height = '50%',out.width = '50%', fig.align='center'}
alpha <- 0.05
ct <- qt(1 - alpha/2, df.residual(score_model))
ct
b1 + c(-1, 1)*ct*seb1
...
\hline
	 ...
	 (Source)        &   ($df$)      &  (SS)             & (MS)         &  F         \\
\hline
Regression & $1$ & $SSR=\sum (\hat{y}_i-\bar{y})^2$ & $MSR=SSR/1$ & $F=\frac{MSR}{MSE}$\\
...
this problem is equivalent to test:
$$H_0:\beta_1=0 \quad vs \quad H_a: \beta_1\neq 0. $$

- Under $H_0$, the model becomes: $y_i=\beta_0+\epsilon_i,$

- and the LS estimate of $\beta_0$ is the one that minimize: $\sum (y_i-\beta_0)^2$.

The minimizer is given by: $b_0=\bar{y}$.

- The residual sum of squares is:
$$SSE_{\text{reduced}}=\sum (y_i-\beta_0)^2=\sum (y_i-\bar{y})^2$$

with $df=n-1$. This is also called the reduced model.


## ANOVA

- Under $H_a$, we have: 
$$SSE_{\text{full}}=\sum (y_i-\hat{y})^2$$
with $df=n-2$. Hence, the reduction in the variability due to the regression is:
$$SSR=SSE_{\text{reduced}}-SSE_{\text{full}},$$
with $df=(n-1)-(n-2)=1$. 
...
If $H_0$ is right, then $SSR$ should be small. Equivalently, if $H_a$ is right, then $SSR$ should be large. How small (large) is small (large)? We compute the following statistic:
$$F=\frac{SSR}{\hat{\sigma}^2}=\frac{SSR/1}{MSE}=\frac{MSR}{MSE}.$$
Under the normal assumption,$F\sim F(1, n-2)$.

- We will reject $H_0$ if: 
$$F>F(1, n-2, \alpha).$$
where $\alpha$ is the significant level, which is the probability allowed to make type-I error.

- More specifically, The most common choice for $\alpha$ is 0.05, which would mean that, were the $H_0$ to be true, we would incorrectly find evidence against it about $5\%$ of the time.

## ANOVA: F test

The ANOVA test of $\beta_1=0$ vs. $\beta_1\neq 0$ is an example of the general test for linear statistical model. 

Summary of general linear test approach:

1. Fit the full model and obtain the error sum of squares $SSE_{full}$.
2. Fit the reduced model under $H_0$ and obtain the error sum of squares $SSE_{reduced}$.
3. Make your decision based on the test statistic.
