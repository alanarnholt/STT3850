...
Term1 <- Term2 %>% 
  select(EDUCATION, LNFACE, LNINCOME, NUMHH)
```
\normalsize
...
\footnotesize
```{r}
modelX1 <- lm(LNFACE ~ LNINCOME, data = Term2)
summary(modelX1)
```
\normalsize
...
eis <- resid(modelX1)
SSE <- sum(eis^2)
SST <- sum((Term2$LNFACE - mean(Term2$LNFACE))^2)
SSR <- SST-SSE
c(SSE, SSR)
...
\footnotesize
```{r}
modelX2 <- lm(LNFACE ~ EDUCATION, data = Term2)
summary(modelX2)
```
\normalsize
...
\footnotesize
```{r}
modelX1X2 <- lm(LNFACE ~ LNINCOME + EDUCATION, data = Term2)
summary(modelX1X2)
```
\normalsize
...
\scriptsize
```{r}
modelAll <- lm(LNFACE ~ LNINCOME + EDUCATION + NUMHH, data = Term2)
summary(modelAll)
```
\normalsize
...
We can use the $\pv$ on the variable to test $H_0:\beta_3=0$.
...
\normalsize
```{r}
modelInt <- lm(LNFACE ~ 1, data = Term2)
summary(modelInt)
```
\normalsize
...
Users of the $R^2_{\text{adj}}$ criterion seek to find a few subsets for which $R^2_{\text{adj}}$ is at the maximum or that adding more variable is not worthwhile.
...
library(MASS)
null <- lm(LNFACE  ~ 1, data = Term1)
full <- lm(LNFACE ~ ., data = Term1)
...
\scriptsize
```{r}
model.BP <- lm(BP ~ ., data = bloodpress)
summary(model.BP)
```
\normalsize
...
VIF for Weight of 8.42 tells you that the variance of the coefficient of Weight is $742\%$ bigger than what you would expect if there was no multicollinearity. 


- Regressing Weight on the remaining five predictors, gives $R^2_{Weight}=88.12\%$. $$VIF_{Weight}=\frac{1}{1-R^2_{Weight}}=\frac{1}{1-0.8812}=8.42$$
...
\tiny
```{r}
model_BPnew <- lm(BP ~ Age + Weight + Dur + Stress, data = bloodpress)
summary(model_BPnew)
vif(model_BPnew)
```
\normalsize
