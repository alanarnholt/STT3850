...
```{r, label = "base1", fig.cap = "Base R scatterplot of `sales` versus `TV`"}
...
```{r, fig.width = 12, fig.height = 4, fig.cap = "Using `grid.arrange()` with `ggplot`"}
library(gridExtra)
...
```{r, fig.width = 12, fig.height = 4, fig.cap = "Using `patchwork` with `ggplot`"}
library(patchwork)
p1 + p2 + p3
```
...
$$\hat{Y_h} \sim N(Y_h = X_h\beta, \sigma^2\mathbf{X_h}(\mathbf{X'X})^{-1}\mathbf{X_h'})$$
For a vector of given values $(\mathbf{X_h})$, a $(1 - \alpha)\cdot 100\%$ confidence interval for the mean response $E(Y_h)$ is

$$CI_{1-\alpha}\left[E(Y_h)\right] = \left[\hat{Y_h} - t_{1 - \alpha/2;n - p - 1}\cdot s_{\hat{Y}_h}, \hat{Y_h} + t_{1 - \alpha/2;n - p - 1}\cdot s_{\hat{Y}_h}  \right]$$
The function `predict()` applied to a linear model object will compute $\hat{Y_h}$ and $s_{\hat{Y_h}}$ for a given $\mathbf{X}_h$.  `R` output has $\hat{Y_h}$ labeled `fit` and $s_{\hat{Y_h}}$ labeled `se.fit`.
...
```{r, label = "mucho", echo = FALSE, fig.cap = "3-D residuals and fitted plane"}
...
```{r, label = "GGDWDYE", fig.cap = "Balance versus Limit"}
...
```{r, label = "CG", fig.cap = "Figure this one out!"}
...
```{r, label = "NLR", fig.cap = "Showing non-linear relationships"}
library(ISLR)
