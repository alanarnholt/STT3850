...
PX[5:7] # Probabilities for categories 4, 5, and 6 or more
```
...
The $\pv = `r pchisq(chi.obs,6, lower = FALSE)`$.
...
- Since the $\pv  = `r p.val`$ is greater than 0.05, fail to reject $H_0$.
...
\begin{tcolorbox}
Given the $\pv$ of $`r CST$p.value`$, evidence suggests birthdays for World Cup soccer players are not uniformly distributed.
\end{tcolorbox}
...
ps <- c(p1, p2, p3, p4)
ps
sum(ps) # check to make sure sum is 1
```
...
\begin{tcolorbox}
If you test using $\alpha = 0.05$, you will fail to reject the null hypothesis since the $\pv = `r pvalue`  > \alpha = 0.05$.  There is not convincing evidence that the data do not come from an Exp($\lambda = 1$).
\end{tcolorbox}
...
The chi-squared approximation is generally satisfactory if the $E_{ij}$s ($\hat{\mu}_{ij}$s) in the test statistic are not too small.  Various rules of thumb exist for what might be considered too small.  One rule of thumb is to require all $E_{ij}$s to be 5 or more.  This can be accomplished by combining cells with small $E_{ij}$s and reducing the overall degrees of freedom.
...
```{r}
xtabs(~Pop + Status, data = NT) -> T3
T3
(E <- chisq.test(T3)$expected)
```
...
**Statistical Conclusion**---The $\pv$ is `r pchisq(chi.obs, 2, lower = FALSE)`.
