...
There were a total of $n = `r sum(Observed)`$ players considered for this study, so the expected count for each quarter is $528/4 = `r 528/4`$.

```{r}
chisq.test(Observed, p = c(1/4, 1/4, 1/4, 1/4))
```

Given the _p-value_ of `r round(chisq.test(Observed, p = c(1/4, 1/4, 1/4, 1/4))$p.value,2)` evidence suggests birthdays for World Cup soccer players are not uniformly distributed.
