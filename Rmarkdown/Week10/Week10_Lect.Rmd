...
- Let $X_1, \ldots, X_n\sim \Gamma(\alpha = 1, \lambda = 1/2)$. 
  ...
  - It is a fact that the sampling distribution of the mean $\bar{X}$ is $\Gamma(n\alpha = 16\cdot 1 = 16, n\lambda = 16/2 = 8)$. 
      
- We draw a random sample of size $n=16$ from a $\Gamma(\alpha=1, \lambda=1/2)$ (population mean: $\alpha/\lambda= 2$, standard deviation: $\sqrt{\alpha/\lambda^2 = 1/(1/2)^2)}=2$).
...
- Given that our bootstrap distribution based on 1000 resamples with replacement in Figure 1 is normally shaped, 

  - let’s use this fact about normal distributions to construct a confidence interval in a different way.

  - First, note that the bootstrap distribution has a mean equal to `r round(mean(bs_dist$stat),2)` (using `infer` or `r round(mean(bm),2)` the for loop). 

  - This value almost coincides exactly with the value of the sample mean $\bar{x}$  of our original 50 pennies of `r mean(pennies_sample$year)`.
...
- Going back to our fishing analogy, this is like asking, “Did our net capture the fish?”.
  ...
  - So, for example, does our percentile-based confidence interval of (`r CI$lci`, `r CI$uci`) "capture” the true mean year $\mu$ of all US pennies? 
...
In this case the $95\%$ confidence interval for $p$, (`r percentile_ci_1$lower_ci`, `r percentile_ci_1$upper_ci`),  contains the true value of 0.375. 
...
- The means of the basic and extended channel commercial times are `r ct[1]` and `r ct[2]` min.

- Is this difference of `r ct[1] - ct[2]` min. statistically significant?
...
- Figure a shows the bootstrap distribution of the difference of sample means. 
    ...
    - We may quantify this variation by computing the bootstrap standard error, which is `r SEbdm`. Again, the bootstrap standard error is the standard error of the sampling distribution.
...
- The $95\%$ bootstrap percentile confidence interval for the difference in means (basic- extended) is (`r CI[1]`, `r CI[2]`). 

    - Thus, we are $95\%$ confident that commercial times on basic channels are, on average, between `r CI[1]` and `r CI[2]` min. 
...
- The distribution is centered around the sample mean of `r mean(CLECmean)`, has a much larger spread due to the small sample size,  with the bootstrap standard error of `r sd(CLECmean)` and a $95\%$ bootstrap percentile interval of (`r CIC[1]`, `r CIC[2]`). 
...
- The bootstrap distribution for the difference in means is shown in Figure a. 
    ...
    - The mean of the bootstrap distribution is `r mean(diffmeans)` with a standard error of `r sd(diffmeans)`. 
    - A $95\%$ bootstrap percentile confidence interval for the difference in means (ILEC-CLEC) is given by (`r CIdiff[1]`, `r CIdiff[2]`).
...
- Compared to the bootstrap difference in ordinary means, this distribution has a much smaller spread. 
...
Compute a $95\%$ confidence interval for $p_{seed}-p_{control}$ using the percentile method. 
...
Compute a $95\%$ confidence interval for $p_{seed}-p_{control}$ using the standard error method. 
...
- We can only use the standard-error method if the bootstrap distribution is bell-shaped (i.e., normally distributed).

- If the sampling distribution is normally shaped, there is another method for constructing confidence intervals that does not involve using your computer.
...
- However, in deriving the confidence interval for $\mu$, we used the fact that $\frac{(\bar{X}-\mu)}{\sigma/\sqrt{n}}$ follows a standard normal distribution. 
