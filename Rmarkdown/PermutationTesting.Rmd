...
In this section, we'll now show you how to use the `infer` package to test a hypothesis. Steps include: `specify()` , `hypothesize()`, `generate()`,  `calculate()`, and `visualize()`, as can be seen in Figure \@ref(fig:inferht).
...
visualize(some) + shade_p_value(obs_stat = obs_diff, direction = "greater")
```
...
visualize(some) + shade_p_value(obs_stat = obs_diff, direction = "two-sided")
```
...
Test whether the proportion of females is 50%. 
...
gss %>% 
  ...
  generate(reps = 1000) %>% 
  calculate(stat = "prop") -> expP
visualize(expP) + shade_p_value(obs_stat = 0.474, direction = "two-sided")
expP %>% get_p_value(obs_stat = 0.474, direction = "two-sided")
```
