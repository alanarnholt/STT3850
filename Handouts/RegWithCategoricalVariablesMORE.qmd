...
mod_simple <- lm(lifeexp ~ gdppercap + continent, data = gapminder1982)
...
summary(modpar)
library(moderndive)
get_regression_table(mod_simple) |> knitr::kable()
ggplot(data = gapminder1982, aes(x = gdppercap, y = lifeexp, color = continent)) + 
  geom_point() + 
  geom_parallel_slopes(se = FALSE) + 
  theme_bw()
get_regression_table(mod_full) |>  knitr::kable()
summary(mod_full)
ggplot(data = gapminder1982, aes(x = gdppercap, y = lifeexp, color = continent)) + 
  geom_point() + 
...
ggplot(data = gapminder1982, aes(x = gdppercap, y = lifeexp, color = continent)) + 
  ...
  geom_smooth(method = "lm", se = FALSE) + 
  theme_bw() + 
  geom_abline(intercept = coef(mod_full)[1], slope = coef(mod_full)[2], color = "red", linetype = "dashed") + 
  geom_abline(intercept = coef(mod_full)[1] + coef(mod_full)[3], slope = coef(mod_full)[2] + coef(mod_full)[7], color = "green", linetype = "dashed") +
  geom_abline(intercept = coef(mod_full)[1] + coef(mod_full)[4], slope = coef(mod_full)[2] + coef(mod_full)[8], color = "darkgreen", linetype = "dashed") + 
  geom_abline(intercept = coef(mod_full)[1] + coef(mod_full)[5], slope = coef(mod_full)[2] + coef(mod_full)[9], color = "blue", linetype = "dashed") + 
  geom_abline(intercept = coef(mod_full)[1] + coef(mod_full)[6], slope = coef(mod_full)[2] + coef(mod_full)[10], color = "purple", linetype = "dashed") 
```
