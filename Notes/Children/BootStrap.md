...
```r
ggplot(data = data.frame(x = my.boot.statB), aes(x = x)) + geom_density(fill = "lightblue") + 
    labs(x = substitute(paste(bar(X), "* (g)")), title = "Bootstrap Distribution of means \n for NC birth weights") + 
    theme_bw() + geom_vline(mean(Babies$Weight, color = "blue"))
```
...
hist(arsenic.mean, main = "Figure 7a", col = "lightblue", breaks = "Scott", 
