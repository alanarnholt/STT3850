...
The exact distribution of this statistic is an unsolved problem.  It does, however, have approximately a $t$ distribution if the populations are normal.  The difficult part is the degrees of freedom.  The degrees of freedom are given with Welch's approximation:
...
```{r label = "CALC"}
library(PASWR)  # Update to PASWR2 sometime
library(ggplot2)
```
...
```{r NORMALITY}
ggplot(data = Calculus, aes(sample = No.Calculus)) + 
  stat_qq() + 
  ggtitle("No Calculus") + 
  theme_bw()
ggplot(data = Calculus, aes(sample = Yes.Calculus)) + 
  stat_qq() + 
  ggtitle("Yes Calculus") + 
  theme_bw()
```
...
```{r Texas}
site <- "https://raw.githubusercontent.com/alanarnholt/STT3850/gh-pages/DataCSV/TXBirths2004.csv"
