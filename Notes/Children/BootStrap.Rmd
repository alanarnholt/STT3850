```{r setupBootStrap, echo = FALSE}
opts_chunk$set(cache.path = './cache/BootStrap/', fig.path = './figure/BootStrap/')
```
...
```
Another 95% bootstrap percentile confidence interval for the ratio of means (ILEC/CLEC) is (`r boot.ci(boot.obj, type = "perc")$perc[4]`, `r boot.ci(boot.obj, type = "perc")$perc[5]`), so with 95% confidence, the true mean repair times for ILEC customers is between `r boot.ci(boot.obj, type = "perc")$perc[4]` and `r boot.ci(boot.obj, type = "perc")$perc[5]` times less than that for CLEC customers.