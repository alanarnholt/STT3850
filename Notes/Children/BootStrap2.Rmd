...
Recall that the bootstrap estimate of bias is the mean of the bootstrap distribution minus the statistic for the original data, $Bias_{boot}[\hat{\theta}^*] = E[\hat{\theta}^*] - \hat{\theta}$.  

* For the second row, $Bias_{boot}[\hat{\theta}^*] = `r mean(my.boot.stat1)` - `r mean(rs1)` = `r mean(my.boot.stat1) - mean(rs1)`$.  

* For the third row, $Bias_{boot}[\hat{\theta}^*] = `r mean(my.boot.stat2)` - `r mean(rs2)` = `r mean(my.boot.stat2) - mean(rs2)`$.
...
* For the second row, $Bias_{boot}[\hat{\theta}^*] = `r mean(my.boot.statg1)` - `r mean(rsg1)` = `r mean(my.boot.statg1) - mean(rsg1)`$.  

* For the third row, $Bias_{boot}[\hat{\theta}^*] = `r mean(my.boot.statg2)` - `r mean(rsg2)` = `r mean(my.boot.statg2) - mean(rsg2)`$.
