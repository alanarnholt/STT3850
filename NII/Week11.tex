...
\urlstyle{same}
\hypersetup{
  pdftitle={STT 3850 : Week 11},
  pdfauthor={Fall 2023},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{STT 3850 : Week 11}
\author{Fall 2023}
\date{}
\institute{App State}
...
\begin{center}\includegraphics[width=0.7\linewidth,height=0.5\textheight]{Week11_files/figure-beamer/unnamed-chunk-2-1} \end{center}
\normalsize
\end{frame}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{CALCULUS }\SpecialCharTok{\%\textgreater{}\%}\FunctionTok{group\_by}\NormalTok{(calculus) }\SpecialCharTok{\%\textgreater{}\%}\FunctionTok{summarize}\NormalTok{(}\AttributeTok{Mean =} \FunctionTok{mean}\NormalTok{(score), }\AttributeTok{n =} \FunctionTok{n}\NormalTok{(), }\AttributeTok{SD =} \FunctionTok{sd}\NormalTok{(score))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{t.test}\NormalTok{(score}\SpecialCharTok{\textasciitilde{}}\NormalTok{calculus, }\AttributeTok{data =}\NormalTok{ CALCULUS, }\AttributeTok{conf.level =} \FloatTok{0.90}\NormalTok{)}\SpecialCharTok{$}\NormalTok{conf.int}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{get\_ci}\NormalTok{(CI, }\AttributeTok{level =} \FloatTok{0.95}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics[width=0.8\linewidth,height=0.6\textheight]{Week11_files/figure-beamer/unnamed-chunk-6-1} \end{center}
\normalsize
\end{frame}
...
\begin{center}\includegraphics[width=0.8\linewidth,height=0.6\textheight]{Week11_files/figure-beamer/unnamed-chunk-7-1} \end{center}
\normalsize
\end{frame}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{t.test}\NormalTok{(Texas}\SpecialCharTok{$}\NormalTok{Weight }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Texas}\SpecialCharTok{$}\NormalTok{Gender)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Texas }\OtherTok{\textless{}{-}} \FunctionTok{within}\NormalTok{(}\AttributeTok{data =}\NormalTok{ Texas, }\AttributeTok{expr=}\NormalTok{\{Gender }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(Gender, }\AttributeTok{levels =}\FunctionTok{c}\NormalTok{(}\StringTok{"Male"}\NormalTok{, }\StringTok{"Female"}\NormalTok{))\})}
\FunctionTok{t.test}\NormalTok{(Texas}\SpecialCharTok{$}\NormalTok{Weight }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Texas}\SpecialCharTok{$}\NormalTok{Gender)}
\end{Highlighting}
\end{Shaded}
...
\[p=\hat{p}\pm z_{\alpha/2}\sqrt{\frac{p(1-p)}{n}}\]
...
$$p=0.59\pm 1.96\sqrt{\frac{0.59(0.41)}{846}}=(0.5569,0.6231)$$
\end{tcolorbox}
\end{frame}
...
\[p=\tilde{p}\pm z_{\alpha/2}\sqrt{\frac{\tilde{p}(1-\tilde{p})}{\tilde{n}}}\]
\end{frame}
...
$$p=0.5896\pm 1.96\sqrt{\frac{0.5896(0.4104)}{850}}=(0.5565,0.6226)$$
\end{tcolorbox}
\end{frame}
...
below \[p=\hat{p}\pm z_{\alpha/2}\sqrt{\frac{p(1-p)}{n}}\]
...
\item
  ...
  \[p=\frac{\hat{p}+\frac{z^2_{\alpha/2}}{2n}\pm z_{\alpha/2}\sqrt{\frac{\hat{p}(1-\hat{p})}{n}+\frac{z^2_{\alpha/2}}{4n^2}}}{1+\frac{z^2_{\alpha/2}}{n}}\]
\end{itemize}
\end{frame}
...
\begin{center}\includegraphics[width=0.6\linewidth,height=0.4\textheight]{Week11_files/figure-beamer/unnamed-chunk-12-1} \end{center}
\normalsize
...
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight]{Week11_files/figure-beamer/unnamed-chunk-17-1} \end{center}
\normalsize
\end{frame}
...
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight]{Week11_files/figure-beamer/unnamed-chunk-19-1} \end{center}
\normalsize
\end{frame}
...
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight]{Week11_files/figure-beamer/unnamed-chunk-21-1} \end{center}
\normalsize
\end{frame}
...
\begin{center}\includegraphics[width=0.6\linewidth,height=0.4\textheight]{Week11_files/figure-beamer/unnamed-chunk-23-1} \end{center}
\normalsize
...
\begin{center}\includegraphics[width=0.9\linewidth,height=0.8\textheight]{Week11_files/figure-beamer/unnamed-chunk-26-1} \end{center}
\normalsize
\end{frame}
...
The quantiles \(Q_1\) and \(Q_2\) are unknown, but they can be estimated
using quantiles of the bootstrap distribution of the \(t\) statistic:
\(T^*=(\bar{X}^*-\bar{x})/(S^*/\sqrt{n})\) where \(\bar{X}^*\) and
\(S^*\) are the mean and standard deviation of a bootstrap resample.
\end{frame}
...
\begin{center}\includegraphics[width=0.8\linewidth,height=0.45\textheight]{Week11_files/figure-beamer/unnamed-chunk-29-1} \end{center}
\normalsize
\end{frame}
...
\begin{center}\includegraphics[width=0.8\linewidth,height=0.45\textheight]{Week11_files/figure-beamer/unnamed-chunk-30-1} \end{center}
\normalsize
...
\begin{center}\includegraphics[width=0.8\linewidth,height=0.45\textheight]{Week11_files/figure-beamer/unnamed-chunk-34-1} \end{center}
\normalsize
\end{frame}
...
\begin{center}\includegraphics[width=0.8\linewidth,height=0.45\textheight]{Week11_files/figure-beamer/unnamed-chunk-35-1} \end{center}
\normalsize
\end{frame}
...
\item
  ...
  \item
    ...
    the bootstrap \(t\) otherwise;
  \item
    the reason being in large samples, we should put more trust in the
    data, in this case, the bootstrap \(t\) is preferred
  \end{itemize}
\item
  Also, the bootstrap percentile makes less of a skewness correction
  than does the bootstrap \(t\). For larger samples, the bootstrap \(t\)
  is preferred.
\end{itemize}
\end{frame}
...
\item
  ...
  \tightlist
  \item
    We will expand further on these ideas here and also provide a
    general framework for understanding hypothesis tests.
  \end{itemize}
\end{itemize}
\end{frame}
...
\item
  ...
  \end{itemize}
\item
  Since only (binary) gender varied from résumé to résumé, researchers
  could isolate the effect of this variable in promotion rates.
\end{itemize}

Note: this study was conducted at a time where more nuanced views of
gender were not as prevalent.
\end{frame}
...
\begin{center}\includegraphics[width=0.7\linewidth,height=0.45\textheight]{Week11_files/figure-beamer/unnamed-chunk-38-1} \end{center}
\normalsize
...
\item
  ...
  résumés by hand?
\item
  One way would be by using standard deck of 52 playing cards,
\end{itemize}
...
\begin{center}\includegraphics[width=0.7\linewidth,height=0.4\textheight]{Week11_files/figure-beamer/unnamed-chunk-43-1} \end{center}
\normalsize
...
\item
  ...
  exercise.
\item
  The distribution of the difference in promotion rates in shown in
  Figure below.
  ...
  \item
    ...
    of \(0.292 = 29.2\%\) with a red line.
  \end{itemize}
\end{itemize}

\begin{center}\includegraphics[width=0.7\linewidth,height=0.4\textheight]{week12_4} \end{center}
\end{frame}
...
\begin{itemize}
\item
  Observe first that the histogram is roughly centered at 0.
...
\item
  ...
  \tightlist
  \item
    this is because even in a hypothesized universe of no gender
    discrimination, you will still likely observe small differences in
    promotion rates because of chance \textbf{sampling variation}.
  \item
...
\item
  ...
  \tightlist
  \item
    While the bootstrap method involves resampling with replacement,
  \item
...
\item
  ...
  somewhat inconsistent with our hypothesized universe.
\item
...
\item
  ...
  \tightlist
  \item
    \(H_0\) is that there is no difference in promotion rate
  \item
    \(H_A\) to be that men are promoted at a higher rate, a subjective
    choice reflecting a prior suspicion we have that this is the case.
  \end{itemize}
\item
...
We can re-express the formulation of our hypothesis test using the
...
To keep things simple for now, we'll stick with the simpler one-sided
alternative.
\end{frame}
...
\item
  ...
  formula used for hypothesis testing.
  ...
  \item
    Here, the samples would be the \(n_m=24\) résumés with male names
    and the \(n_f\) résumés with female names.
  \item
...
\item
  ...
  \tightlist
  \item
    In our case, how will the difference in sample proportions
    \(\hat{p}_m-\hat{p}_f\).
  \item
...
Then, the null distributions is the sampling distributions assuming the
null hypothesis \(H_0\) is true.
\end{frame}

\begin{frame}{Understanding hypothesis tests}
\protect\hypertarget{understanding-hypothesis-tests-7}{}
We previously visualized a null distribution, which we re-display below:

\begin{center}\includegraphics[width=0.6\linewidth,height=0.4\textheight]{week12_6} \end{center}
\end{frame}
...
\item
  ...
  \item
    ...
    observed?
  \item
    In our case, 0 times out of 16, we obtained a difference in
    proportion greater than or equal to the observed difference of 0.292
    = 29.2\%.
  \item
...
Seventh and lastly, in many hypothesis testing procedures, it is
commonly recommended to set the \textbf{significance level} of the test
beforehand.
...
\item
  ...
  \tightlist
  \item
    where if the \(p\)-value falls below \(\alpha\) we would ``reject
    the null hypothesis \(H_0\).
  \item
    if the \(p\)-value does not fall below \(\alpha\), we would ``fail
    to reject \(H_0\).
  \end{itemize}
\item
...
\item
  ...
  \begin{itemize}
  \item
    with confidence intervals we generated replicates using
    \texttt{type\ =\ "bootstrap"} resampling with replacement,
  \item
    we'll now perform shuffles/permutations by setting
    \texttt{type\ =\ "permute"}, resampling \textbf{without} replacement
  \end{itemize}
\end{itemize}
...
\begin{center}\includegraphics[width=0.7\linewidth,height=0.45\textheight]{Week11_files/figure-beamer/unnamed-chunk-54-1} \end{center}
\normalsize
\end{frame}
...
\begin{center}\includegraphics[width=0.7\linewidth,height=0.35\textheight]{Week11_files/figure-beamer/unnamed-chunk-58-1} \end{center}
\normalsize
...
\item
  ...
  \item
    ...
    don't have enough evidence to say so.'' There is an absence of
    enough proof.
  \item
